...
def get_mask(shape: Tuple[int, int]) -> torch.Tensor:
    ...
    mask = torch.from_numpy(p_mask >= np.random.uniform(low=0, high=1, size=shape[0] * shape[1])).reshape(
        (shape[0], shape[1]))
    return mask.float()
la10 = getEXR('./HRinput/label_10.exr')
la10 = torch.from_numpy(la10).permute(2,0,1).unsqueeze(0)#1 3 768 1024
mask = get_mask(shape=(la10.shape[2], la10.shape[3]))

HRmaskinput = la10 * mask
save_image(
                        HRmaskinput,
                        fp=os.path.join('./HRinput/HRmaskinput.exr'),
                        format='exr',
                        nrow=1)